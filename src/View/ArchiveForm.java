...
public class ArchiveForm extends JPanel {
    ...
    private JPanel buildCenter() {
        ...
        bar.add(UITheme.searchField(txtSearch, "Tìm theo ID gốc hoặc nội dung JSON..."));
    ...
    private void loadData() {
        ...
        if ("Hội viên".equals(tab)) {
            sql = "SELECT id, maHoiVien, tenHoiVien, trangThai, thoiGianLuuTru FROM HoiVien WHERE daLuuTru=1 "
                + (kw.isEmpty() ? "" : " AND (maHoiVien LIKE ? OR tenHoiVien LIKE ?) ")
                + " ORDER BY thoiGianLuuTru DESC";
        } else if ("Hoạt động".equals(tab)) {
            sql = "SELECT id, tenHoatDong, loaiHoatDong, trangThai, thoiGianLuuTru FROM HoatDong WHERE daLuuTru=1 "
                + (kw.isEmpty() ? "" : " AND (tenHoatDong LIKE ? OR loaiHoatDong LIKE ?) ")
                + " ORDER BY thoiGianLuuTru DESC";
        } else {
            sql = "SELECT tg.id, hv.tenHoiVien, hd.tenHoatDong, tg.trangThai, tg.thoiGianLuuTru "
                + "FROM ThamGia tg JOIN HoiVien hv ON tg.idHoiVien=hv.id JOIN HoatDong hd ON tg.idHoatDong=hd.id "
                + "WHERE tg.daLuuTru=1 "
                + (kw.isEmpty() ? "" : " AND (hv.tenHoiVien LIKE ? OR hd.tenHoatDong LIKE ?) ")
    ...
    private void restoreSelected() {
        ...
        String sql;
        if ("Hội viên".equals(loai)) sql = "UPDATE HoiVien SET daLuuTru=0, thoiGianLuuTru=NULL WHERE id=?";
        else if ("Hoạt động".equals(loai)) sql = "UPDATE HoatDong SET daLuuTru=0, thoiGianLuuTru=NULL WHERE id=?";
        else sql = "UPDATE ThamGia SET daLuuTru=0, thoiGianLuuTru=NULL WHERE id=?";
        try (Connection c = DatabaseHelper.getConnection()) {
            PreparedStatement ps = c.prepareStatement(sql);
            ps.setInt(1, id);
            ps.executeUpdate();
    ...
    private void deleteForeverSelected() {
        ...
        String sql = "Hội viên".equals(loai) ? "DELETE FROM HoiVien WHERE id=?" : ("Hoạt động".equals(loai) ? "DELETE FROM HoatDong WHERE id=?" : "DELETE FROM ThamGia WHERE id=?");
        try (Connection c = DatabaseHelper.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
